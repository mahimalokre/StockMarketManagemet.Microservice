...
using StockMarket.StockPriceApi.Consumer;
using StockMarket.StockPriceApi.Data;
...
builder.Services.AddMassTransit(x =>
{
    ...
    x.AddBus(provider => Bus.Factory.CreateUsingRabbitMq(config =>
    {
        ...
        config.ReceiveEndpoint("CompanyCodeQueue", queue =>
        {
            queue.ConfigureConsumer<CompanyConsumer>(provider);
        });
    }));
});
